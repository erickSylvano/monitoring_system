...
export default function Statuses({ auth, statuses, types }) {
    ...
    useEffect(() => {
        ...
        hostChannel.listen('.host.created', (data) => {
            ...
            const newHostStatus = {
                id: data.host.id,
                type: { type_name: 'PC' }, // Ajoutez un type pour identifier cet événement
                host: data.host,
            };
        ...
        // Mettez à jour la map de l'existence de l'état HDD lors de la réception des données
    ...
    const filteredStatuses = sortStatuses(
        statusList.filter((status) => {
            if (
                (status.host && status.type && status.type.type_name === 'PC') ||
                (status.type && status.type.type_name === 'Imported')
            ) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Liste des PC</h2>}
        >
            <Head title="PC" />
        <section>
                            ...
                            <div className="overflow-x-auto mt-4">
                                <table className="table-auto w-full">
                                    ...
                                    {filteredStatuses.map(({ id, state, type, host, last_updated_at }) => {
                                        ...
                                        const now = Moment();
                                        const isOfflineOver = state === 'Hors ligne' && now.diff(lastUpdate, 'hours') >= 1;
                                            ...
                                            >
                                                ...
                                                <td className="whitespace-nowrap px-4 py-2">
                                                    <div className="flex items-center">
                                                        ...
                                                        {state === 'Hors ligne' && (
                                                            // <span className={`w-3 h-3 rounded-full inline-block mr-1 ${isOfflineOver ? 'bg-red-500' : 'bg-gray-500'}`}></span>
